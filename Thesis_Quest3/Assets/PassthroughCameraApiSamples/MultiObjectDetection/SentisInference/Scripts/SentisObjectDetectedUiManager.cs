...
namespace PassthroughCameraSamples.MultiObjectDetection
{
    ...
    public class SentisObjectDetectedUiManager : MonoBehaviour
    {
        ...
        {
            // Position the canvas in front of the camera and face it with world-up to avoid roll/tilt
            if (m_cameraAccess == null || !m_cameraAccess.IsPlaying)
            {
                return;
            }
            var camPose = m_cameraAccess.GetCameraPose();
            m_captureCameraPose = camPose;
            var forward = camPose.rotation * Vector3.forward;
            m_capturePosition = camPose.position + forward * m_canvasDistance;
            m_captureRotation = Quaternion.LookRotation(forward, Vector3.up);
            m_detectionCanvas.transform.SetPositionAndRotation(m_capturePosition, m_captureRotation);
        }

        private void LateUpdate()
        {
            // Continuously face the camera each frame so UI stays front-facing even when no detections are drawn
            UpdatePosition();
        }
        ...
        public void CapturePosition()
        {
            ...
            var forward = m_captureCameraPose.rotation * Vector3.forward;
            m_capturePosition = m_captureCameraPose.position + forward * m_canvasDistance;
            m_captureRotation = Quaternion.LookRotation(forward, Vector3.up);
        }
