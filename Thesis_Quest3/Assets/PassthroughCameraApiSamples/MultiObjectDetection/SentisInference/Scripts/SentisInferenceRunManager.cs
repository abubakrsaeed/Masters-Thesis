...
namespace PassthroughCameraSamples.MultiObjectDetection
{
    ...
    public class SentisInferenceRunManager : MonoBehaviour
    {
        ...
        private float m_captureStartTime = -1f;   // capture -> draw
        private float m_inferenceStartTime = -1f; // schedule -> draw
        public float LastInferenceMs { get; private set; }
        public float LastEndToEndMs { get; private set; }
        ...
        public void RunInference(PassthroughCameraAccess cameraAccess)
        {
            ...
            if (!m_started)
            {
                ...
                m_captureStartTime = now;
                m_inferenceStartTime = -1f; // will start when we begin executing layers
        ...
        private void InferenceUpdate()
        {
            ...
            if (m_started)
            {
                try
                {
                    ...
                    {
                        if (m_inferenceStartTime < 0f)
                        {
                            m_inferenceStartTime = Time.realtimeSinceStartup; // mark when compute begins
                        }
                        var it = 0;
        ...
        private void GetInferencesResults()
        {
            ...
            switch (m_download_state)
            {
                ...
                case 3:
                    ...
                    {
                        var now = Time.realtimeSinceStartup;
                        LastInferenceMs = (now - m_inferenceStartTime) * 1000f;
                        LastEndToEndMs = m_captureStartTime > 0f ? (now - m_captureStartTime) * 1000f : LastInferenceMs;
                        m_inferenceStartTime = -1f;
                        m_captureStartTime = -1f;
                    }
                    m_download_state = 5;
                    break;
                case 4:
